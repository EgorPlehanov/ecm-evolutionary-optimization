...
**Терминология DE.** Под *популяцией* понимается набор из $N$ кандидатных решений (векторов параметров $\log_{10} B_1, \log_{10} B_2$), эволюционирующих от поколения к поколению. *Мутант* – промежуточный вектор, получаемый как $v = x_{r1} + F \cdot (x_{r2} - x_{r3})$, где $x_{r1}, x_{r2}, x_{r3}$ – три случайно выбранных различных вектора из популяции, а $F$ – коэффициент масштабирования (обычно в диапазоне $[0,2]$). *Кроссовер* – оператор смешивания: trial-вектор $u$ формируется путём заимствования компонент у мутанта $v$ или у исходного индивида $x_i$ в зависимости от вероятности кроссовера $CR \in [0,1]$. *Жадный отбор* – сравнение целевой функции (fitness) trial-вектора и исходного индивида: если $u$ лучше (имеет меньшее значение score), он заменяет $x_i$ в следующем поколении; в противном случае $x_i$ сохраняется. Такой механизм обеспечивает баланс исследования (за счёт мутации) и эксплуатации (за счёт отбора) без вычисления градиента.
