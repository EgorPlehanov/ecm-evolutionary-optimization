...
Для обновления каждого целевого вектора $x_i^{(g)}$ используется классическая стратегия DE/rand/1. На этом шаге из текущей популяции выбираются три попарно различных индивида $x_{r1}^{(g)},x_{r2}^{(g)},x_{r3}^{(g)}$, не совпадающих с обновляемым решением. На их основе строится мутантный вектор
$v_i^{(g)}=x_{r1}^{(g)} + F\cdot\big(x_{r2}^{(g)}-x_{r3}^{(g)}\big)$,
где параметр $F\in(0,2)$ определяет масштаб дифференциального возмущения. Затем мутантный вектор комбинируется с исходным решением посредством покомпонентного кроссовера: пробный вектор $u_i^{(g)}$ получает координаты из $v_i^{(g)}$ с вероятностью $CR$ и из $x_i^{(g)}$ с вероятностью $1-CR$. Параметр $CR\in[0,1]$ тем самым задаёт интенсивность рекомбинации. После построения кандидата для него вычисляется значение $\mathrm{score}(u_i^{(g)})$ во внешнем запуске GMP-ECM. Переход к следующему поколению выполняется по правилу жадного отбора: если пробный вектор оказывается не хуже родительского, принимается $x_i^{(g+1)}=u_i^{(g)}$; иначе в популяции сохраняется прежнее значение $x_i^{(g+1)}=x_i^{(g)}$.
...
Генетический алгоритм (Genetic Algorithm, GA) относится к эволюционным методам оптимизации и опирается на последовательное применение селекции, скрещивания и мутации. В рассматриваемой постановке он используется с вещественным кодированием кандидатов, поскольку параметры $B_1$ и $B_2$ подбираются в непрерывном логарифмическом пространстве, введённом в начале раздела 2.3. Дополнительно в алгоритм включается элитизм: несколько лучших особей текущего поколения переносятся в следующее поколение без изменения.

Как и в случае DE, отдельный индивид задаёт пару $x_i=(\log_{10}B_1,\log_{10}B_2)$, а его приспособленность определяется внешней оценкой GMP-ECM на обучающей выборке. Отличие заключается в способе получения новых точек. Если дифференциальная эволюция использует разности между векторами популяции, то GA строит кандидатов за счёт рекомбинации родительских решений. Благодаря этому удачные компоненты разных особей могут сочетаться в одном потомке, например подходящий масштаб $B_1$ может быть объединён с перспективным отношением $B_2/B_1$.

Начальная популяция $P^{(0)}$ генерируется случайно в пределах заданных границ логарифмического пространства. После вычисления значений score для всех особей начинается формирование нового поколения. Выбор родителей выполняется турнирной селекцией: из популяции случайно отбирается небольшая группа кандидатов, после чего родителем назначается особь с наименьшим значением score среди участников турнира. Такой отбор создаёт преимущество для более качественных решений, но сохраняет ненулевую вероятность участия менее сильных особей, что важно для поддержания разнообразия популяции.

Для пары выбранных родителей $p_a$ и $p_b$ применяется вещественное арифметическое скрещивание
$c=\alpha p_a+(1-\alpha)p_b$,
где $\alpha\sim U(0,1)$. Полученный потомок $c$ расположен на отрезке между родительскими решениями в логарифмическом пространстве, поэтому он наследует их координаты в усреднённой форме. Затем с вероятностью $p_m$ выполняется гауссова мутация:
$c'=c+\varepsilon$,
где вектор $\varepsilon$ формируется покомпонентно из распределения $\mathcal{N}(0,\sigma^2)$. Эта операция обеспечивает локальное исследование окрестности найденных решений и препятствует слишком раннему сужению популяции.

После рекомбинации и возможной мутации кандидат приводится к допустимой области поиска. Логарифмические координаты ограничиваются заданными диапазонами, а при декодировании в реальные параметры дополнительно проверяются условия $B_2\ge B_1$ и $\frac{B_2}{B_1}\le R_{\max}$. Затем потомок оценивается запуском GMP-ECM. Новое поколение составляется из элитных особей текущей популяции и оценённых потомков; за счёт этого лучшие найденные решения не теряются вследствие случайных операций скрещивания и мутации.

В результате один цикл GA включает выбор родителей с предпочтением более качественных решений, построение промежуточного потомка, локальное случайное возмущение, коррекцию ограничений и перенос лучших кандидатов. Для задачи подбора $B_1,B_2$ такая схема сочетает использование уже найденных перспективных масштабов параметров с исследованием соседних областей пространства поиска.
...
где $\omega$ – коэффициент инерции, $c_1$ и $c_2$ – веса когнитивной и социальной компонент, $r_1,r_2\sim U(0,1)$ – независимые случайные множители. Первое слагаемое отвечает за сохранение текущей тенденции движения частицы, второе учитывает её собственный лучший найденный результат, а третье задаёт влияние лучшего решения, обнаруженного роем в целом.

После обновления позиции выполняется приведение кандидата к допустимой области. Координаты ограничиваются заданными диапазонами поиска, а при декодировании дополнительно проверяются условия $B_2\ge B_1$ и $\frac{B_2}{B_1}\le R_{\max}$. При необходимости используется отсечение скорости (velocity clamping), позволяющее ограничить чрезмерно большие перемещения частиц и тем самым сохранить устойчивость поиска в узких перспективных областях.

Далее для новых позиций вычисляется значение score с помощью внешнего запуска GMP-ECM. Если новая позиция улучшает индивидуально лучшее значение частицы, обновляется $pbest_i$. Если же среди всех частиц получено новое наилучшее значение, обновляется глобальный вектор $gbest$. После этого итерации продолжаются до исчерпания заданного лимита по времени или числу вычислений целевой функции.
...
Как и в предыдущих методах, кандидат задаётся в едином пространстве $x=(\log_{10}B_1,\log_{10}B_2)$, а истинное значение score становится известным только после внешнего запуска. Особенность BO состоит в том, что между такими запусками поддерживается статистическая модель $\hat f(x)$, оценивающая для произвольной точки прогнозируемое среднее значение $\mu(x)$ и неопределённость прогноза $\sigma(x)$. В областях, близких к уже проведённым измерениям, апостериорная неопределённость, как правило, ниже; напротив, в удалённых участках пространства поиска или при противоречивых наблюдениях она возрастает. Поэтому выбор очередного кандидата формулируется как компромисс между уточнением уже перспективных зон и исследованием областей, информация о которых пока недостаточна.

В качестве суррогатной модели часто используется гауссовский процесс (Gaussian Process, GP). Он задаётся ковариационным ядром $k(x,x')$, которое описывает предполагаемую связь значений целевой функции в близких точках, а также шумовым параметром, учитывающим стохастическую природу измерений. В данной задаче такая стохастичность связана со случайным выбором кривых ECM, возможными таймаутами и флуктуациями времени выполнения. По набору наблюдений $\mathcal{D}_t=\{(x_i,y_i)\}_{i=1}^{t}$, где $y_i=\mathrm{score}(x_i)$, модель формирует апостериорное распределение прогноза для произвольной точки $x$.

Следующая точка выбирается путём максимизации acquisition-функции $a(x\mid\mathcal{D}_t)$. В работе применяется стандартная для BO логика сочетания exploitation и exploration, характерная, в частности, для критериев Expected Improvement (EI) и Upper/Lower Confidence Bound (UCB/LCB). Первая составляющая отдаёт предпочтение точкам с низким прогнозируемым значением score, то есть с малым $\mu(x)$, тогда как вторая повышает привлекательность областей с большой неопределённостью $\sigma(x)$. Итоговый критерий объединяет оба фактора и тем самым задаёт очередной кандидат $x_{t+1}$.
...
В качестве основного инструментария разработки выбран Python 3: он удобен для построения исследовательских pipeline, имеет развитую экосистему научных библиотек и позволяет в одной системе объединить генерацию данных, запуск внешних процессов, оптимизацию и анализ результатов. GMP-ECM используется как зрелая внешняя реализация ECM и как источник справочных параметров, что позволяет сосредоточить работу на автоматическом выборе $B_1/B_2$, а не на реализации низкоуровневой арифметики эллиптических кривых. Для численных вычислений и оптимизации применяются NumPy и SciPy, для построения графиков и отчетных иллюстраций - matplotlib, а интерфейс командной строки реализован с использованием Click, поскольку длительные эксперименты удобнее запускать воспроизводимыми командами и JSON-планами. Версионирование исходного кода выполняется через Git, а серийные вычисления вынесены в HPC-среду СКЦ СПбПУ под управлением SLURM, так как оценка одной конфигурации требует большого числа независимых запусков GMP-ECM.
...
Выход `Fitness evaluator` сохраняется в JSON-артефакты, которые затем совместно с результатами отдельного этапа валидации передаются в блок анализа. Валидация на схеме выделена отдельной веткой, чтобы подчеркнуть методологическое разделение между оптимизацией на train-наборе и проверкой обобщающей способности на control-наборе. Итогом работы архитектурного контура являются агрегированные отчёты и визуализации в форматах Markdown, CSV и PNG; далее они используются в главе 4 как эмпирическая основа для сравнительного анализа методов.

## 3.3. Генерация датасетов

Модуль генерации датасетов предназначен для воспроизводимого формирования наборов полупростых чисел вида $N = p q$, используемых как на этапе оптимизации, так и при независимой проверке найденных параметров. Конфигурация модуля включает целевую битовую длину простого делителя $p$, размер кофактора $q$, объёмы train- и control-наборов, а также значение `seed`. Последний параметр фиксирует состояние генератора псевдослучайных чисел и тем самым обеспечивает повторяемость эксперимента: при неизменной конфигурации восстанавливаются та же последовательность кандидатов и тот же итоговый состав выборки.

Генерация одного элемента набора выполняется последовательным подбором кандидатов $p$ и $q$ требуемой разрядности, проверкой их простоты и последующим построением числа $N = p q$. После этого контролируется соответствие полученного числа заданным ограничениям по битовой длине. Если кандидат не удовлетворяет хотя бы одному из условий, он исключается из рассмотрения, а процедура повторяется до получения необходимого числа элементов. Сформированный пул затем разделяется на train и control без пересечений, что предотвращает утечку данных между этапом настройки и этапом подтверждения качества.
...
На первом этапе использовались train-count = 80, control-count = 80, max-curves-per-n = 260 на этапе оптимизации и 600 на этапе валидации. Область поиска задавалась как $B_1$ от $100$ до $100000$, $B_2$ от $10000$ до $5000000$, отношение $\frac{B_2}{B_1}$ не более $5000$.
...
Таблица 4.1
Сопоставление справочных параметров GMP-ECM и параметров, найденных для 20-значных делителей
...
При уточнённых границах поиска все рассмотренные методы также превзошли справочную конфигурацию по валидационной метрике и среднему времени выполнения. Снижение валидационной метрики находилось в диапазоне 13,70-25,14%, времени – 14,30-21,04%, а среднего числа кривых – 9,71-62,42%. Наименьшие значения валидационной метрики и времени были получены для GA, тогда как наиболее заметное сокращение числа кривых показал PSO. Метод DE занял устойчивое промежуточное положение, сохранив сбалансированные значения по трём основным показателям. BO также улучшил справочные параметры, однако на этапе уточнения границ его преимущество оказалось менее выраженным, чем в эксперименте с широкой областью поиска.

Сводные статистики показывают, что оба этапа дали сопоставимый общий уровень улучшения, но различались по распределению результатов между методами. Для широких границ медианный относительный выигрыш валидационной метрики составил 22,912%, а медианное итоговое значение целевой функции – 24716,45. Для уточнённых границ соответствующие значения составили 22,355% и 25799,23. Несмотря на близость медианных выигрышей, второй этап привёл к более контрастному ранжированию методов: по итоговым метрикам наиболее показательными оказались GA и PSO.

На рисунках 4.1 и 4.2 представлено сравнение относительного выигрыша валидационной метрики для разных методов. Эти графики позволяют сопоставить, насколько найденные конфигурации улучшают справочные параметры GMP-ECM на независимой контрольной выборке при широких и уточнённых границах поиска.

![](../../data/analysis/analyze_20260509T121523Z_job7040950/overview/groups/divisor_size=20/plots/validation_gain_by_method.png)
Рис. 4.1 Относительный выигрыш валидационной метрики по методам при широких границах поиска

![](../../data/analysis/analyze_20260519T191518Z_job7106009/overview/groups/divisor_size=20/plots/validation_gain_by_method.png)
Рис. 4.2 Относительный выигрыш валидационной метрики по методам при уточненных границах поиска

Рисунки 4.3 и 4.4 отражают соотношение между относительным выигрышем и временем, затраченным на оптимизацию. Такой анализ важен для практического применения, поскольку наибольшее снижение валидационной метрики не всегда достигается при минимальной вычислительной стоимости поиска параметров.

![](../../data/analysis/analyze_20260509T121523Z_job7040950/overview/groups/divisor_size=20/plots/pareto_runtime_gain_by_method.png)
Рис. 4.3 Компромисс между временем оптимизации и выигрышем при широких границах поиска

![](../../data/analysis/analyze_20260519T191518Z_job7106009/overview/groups/divisor_size=20/plots/pareto_runtime_gain_by_method.png)
Рис. 4.4 Компромисс между временем оптимизации и выигрышем при уточненных границах поиска

На рисунках 4.5 и 4.6 приведена динамика сходимости трёх лучших методов. Эти графики характеризуют скорость приближения к лучшей найденной конфигурации и позволяют проследить, сохраняется ли достигнутое улучшение при дальнейшем расходовании вычислительного бюджета.

![](../../data/analysis/analyze_20260509T121523Z_job7040950/overview/groups/divisor_size=20/plots/convergence_ribbons_top3_methods.png)
Рис. 4.5 Динамика сходимости лучших методов при широких границах поиска

![](../../data/analysis/analyze_20260519T191518Z_job7106009/overview/groups/divisor_size=20/plots/convergence_ribbons_top3_methods.png)
Рис. 4.6 Динамика сходимости лучших методов при уточненных границах поиска

Важной частью интерпретации результатов являются графики из отчетов оптимизации отдельных методов. Они показывают не только итоговые значения метрик, но и внутренний ход поиска: какие значения $B_1$ и $B_2$ выбирал оптимизатор, где в пространстве параметров возникали лучшие оценки, на каких этапах снижалось текущее лучшее значение и как найденные параметры проявляли себя на контрольной выборке. Для анализа выбраны графики GA, PSO и DE. GA дал лучший итог по валидационной метрике и времени, PSO показал наибольшее сокращение числа кривых, а DE использован как пример устойчивого популяционного поиска с более выраженной локализацией оценок около перспективной области.

На рисунке 4.7 представлена траектория поиска GA в пространстве параметров $B_1, B_2$ при широких границах. График фиксирует последовательность конфигураций, проверенных оптимизатором в ходе эксперимента. Траектория не концентрируется около справочных параметров GMP-ECM, а смещается в область, где $B_1$ имеет порядок десятков тысяч, а $B_2$ – нескольких миллионов. Это согласуется с численными результатами таблицы 4.1 и указывает на то, что автоматическая оптимизация выделила более эффективную область параметров для рассматриваемого класса чисел.

![](../../data/experiments/20_dset_20260509T075225Z_job7040939/ga/optimize_20260509T083540Z_job7040948/plots/ga_optimize_20260509T083540Z_job7040948_b1_b2_trajectory.png)
Рис. 4.7 Траектория поиска GA в пространстве параметров при широких границах

Рисунок 4.8 дополняет траекторный анализ тепловой картой значений целевой функции в координатах $B_1$ и $B_2/B_1$ для GA. Отдельные точки соответствуют проверенным конфигурациям, а цветовая шкала отражает полученную оценку. Области с меньшими значениями целевой функции образуют локальные зоны интереса; именно такие участки пространства параметров целесообразно рассматривать как кандидаты для последующего уточнения поиска.

![](../../data/experiments/20_dset_20260509T075225Z_job7040939/ga/optimize_20260509T083540Z_job7040948/plots/ga_optimize_20260509T083540Z_job7040948_b1_ratio_heatmap.png)
Рис. 4.8 Тепловая карта оценок GA в координатах $B_1$ и $B_2/B_1$

На рисунке 4.9 приведена динамика улучшения лучшей оценки GA при уточнённых границах поиска. Линия соответствует текущему лучшему значению целевой функции, а столбцы отмечают величину улучшения в те моменты, когда алгоритм находил новую лучшую конфигурацию. Характер графика показывает, что прогресс имел дискретный вид: отдельные успешные шаги сменялись участками плато, что типично для стохастической оптимизации вычислительно затратных функций.

![](../../data/experiments/20_dset_20260519T115357Z_job7105998/ga/optimize_20260519T115430Z_job7106007/plots/ga_optimize_20260519T115430Z_job7106007_jump_plot.png)
Рис. 4.9 Скачки улучшения лучшей оценки GA при уточненных границах поиска

На рисунке 4.10 приведен график прогресса DE при уточненных границах. Он совмещает лучшее найденное значение и неагрегированные оценки, полученные на разных этапах оптимизации. В отличие от случайного поиска, для которого характерна высокая вариативность оценок на протяжении всего процесса, популяционный метод постепенно концентрируется около более перспективных значений параметров, поэтому разброс оценок становится более содержательным для анализа локальной области.

![](../../data/experiments/20_dset_20260519T115357Z_job7105998/de/optimize_20260519T115430Z_job7105999/plots/de_optimize_20260519T115430Z_job7105999_progress_by_phase.png)
Рис. 4.10 Прогресс оптимизации DE при уточненных границах поиска

На рисунке 4.11 показана эффективность GA по времени. График совмещает улучшение лучшего значения целевой функции и скользящую скорость выполнения оценок. Рост числа оценок в секунду на отдельных участках означает, что проверяемые конфигурации становятся дешевле в вычислении; для данной задачи это связано с уменьшением времени факторизации и числа кривых при более удачном выборе параметров.

![](../../data/experiments/20_dset_20260519T115357Z_job7105998/ga/optimize_20260519T115430Z_job7106007/plots/ga_optimize_20260519T115430Z_job7106007_time_efficiency.png)
Рис. 4.11 Эффективность GA по времени при уточненных границах поиска

Для анализа результатов валидации наиболее информативны распределения, поскольку они агрегируют сравнение справочных и оптимизированных параметров по объектам контрольной выборки. Валидационные трассы позволяют рассматривать вклад отдельных чисел и повторов, однако графики распределений дают более компактное представление итогового эффекта. На рисунке 4.12 приведено распределение валидационной метрики для GA. Смещение распределения оптимизированной конфигурации в сторону меньших значений свидетельствует об улучшении качества в терминах минимизируемой метрики.

![](../../data/experiments/20_dset_20260519T115357Z_job7105998/ga/optimize_20260519T115430Z_job7106007/plots/ga_validate_20260519T174715Z_job7106008_score_distribution.png)
Рис. 4.12 Распределение валидационной метрики для эталонных и оптимизированных параметров GA

На рисунке 4.13 представлено распределение времени валидации для GA. Оно показывает, что улучшение итоговой метрики связано не только с изменением числа кривых, но и с уменьшением времени выполнения факторизации на контрольной выборке. Сдвиг распределения оптимизированной конфигурации в область меньших значений подтверждает наличие практического ускорения относительно справочного режима.

![](../../data/experiments/20_dset_20260519T115357Z_job7105998/ga/optimize_20260519T115430Z_job7106007/plots/ga_validate_20260519T174715Z_job7106008_time_distribution.png)
Рис. 4.13 Распределение времени валидации для эталонных и оптимизированных параметров GA

На рисунке 4.14 приведено распределение числа кривых для PSO, поскольку именно этот метод обеспечил наибольшее сокращение соответствующего среднего показателя. Распределение уточняет практическую интерпретацию результата: при сохранении высокой доли успешных факторизаций оптимизированная конфигурация требует заметно меньшего числа кривых по сравнению со справочным режимом GMP-ECM.

![](../../data/experiments/20_dset_20260519T115357Z_job7105998/pso/optimize_20260519T115430Z_job7106003/plots/pso_validate_20260519T182113Z_job7106004_curves_distribution.png)
Рис. 4.14 Распределение числа кривых для эталонных и оптимизированных параметров PSO

## 4.4. Сравнение методов по качеству и стоимости

При выборе метода необходимо учитывать не только относительный выигрыш валидационной метрики, но и вычислительную стоимость самого процесса оптимизации. BO показал высокие результаты при широких границах поиска, однако после уточнения области его преимущество стало менее выраженным. GA оказался наиболее результативным по совокупности итоговых показателей: на этапе широкого поиска он имел лучшее итоговое значение целевой функции, а при уточнённых границах обеспечил максимальное снижение валидационной метрики (-25,14%) и времени выполнения (-21,04%). PSO, в свою очередь, выделяется по критерию числа кривых: при уточнённых границах их среднее количество уменьшилось на 62,42%.

DE продемонстрировал наиболее сбалансированный профиль результатов: снижение валидационной метрики составило 22,91% и 22,35%, времени – 18,90% и 17,92%, а числа кривых – 49,25% и 51,69% для двух этапов эксперимента соответственно. RS, несмотря на простоту процедуры, также улучшил справочную конфигурацию по всем ключевым метрикам. В практическом смысле он может рассматриваться как быстрый ориентир и как нижняя граница эффективности автоматического подбора.

Практическая интерпретация результатов состоит в следующем. Для исследованного класса чисел с 20-значным делителем наиболее предпочтительными являются GA, PSO и DE: GA – по качеству и времени выполнения, PSO – по числу кривых, DE – как сбалансированный вариант. BO целесообразно рассматривать как инструмент исследования широких областей параметров; после сужения диапазона поиска его эффективность в большей степени зависит от настройки бюджета и параметров суррогатной модели.
...
Второе ограничение связано с типом чисел. Использовались полупростые числа $N = pq$ с контролируемым размером p и q. Для специальных классов, например чисел Мерсенна, Ферма, чисел вида $s^n \pm 1$ и кофакторов из реальных NFS-задач, требуется отдельная проверка. Третье ограничение – стохастический характер ECM и оптимизаторов. Результаты следует подтверждать несколькими seed и независимыми control-наборами.
