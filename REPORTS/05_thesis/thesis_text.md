...
На рисунках 4.1 и 4.2 приведено сравнение относительного выигрыша валидационной метрики по методам. Эти графики показывают, насколько каждая найденная конфигурация улучшает эталонные параметры GMP-ECM на независимой контрольной выборке при широких и уточненных границах поиска.
...
На рисунках 4.3 и 4.4 показан компромисс между относительным выигрышем и временем оптимизации. Он важен для практического применения, поскольку максимальное улучшение валидационной метрики не всегда соответствует минимальной стоимости поиска параметров.
...
На рисунках 4.5 и 4.6 представлена динамика сходимости трех лучших методов. По этим графикам можно оценить, насколько быстро метод приближается к лучшей найденной конфигурации и сохраняется ли улучшение в ходе дальнейших вычислений.
...
На рисунке 4.7 показана траектория поиска GA в пространстве параметров $B_1, B_2$ при широких границах. График отражает последовательность значений, которые выбирал оптимизатор в ходе работы. Видно, что метод не сосредоточился у исходных справочных параметров, а сместился в область $B_1$ порядка десятков тысяч и $B_2$ порядка нескольких миллионов. Это согласуется с численными результатами таблицы 4.1 и подтверждает, что автоматическая оптимизация выявила новую эффективную область параметров.
...
На рисунке 4.8 приведена тепловая карта зависимости качества от $B_1$ и отношения $B_2/B_1$ для GA. Точки соответствуют проверенным конфигурациям, а цвет отражает полученную оценку целевой функции. Такой график позволяет визуально определить области, где значения параметров оказываются более перспективными: более низкие значения оценки образуют локальные зоны интереса, которые затем целесообразно исследовать точнее.
...
На рисунке 4.9 показаны моменты улучшения лучшего значения для GA при уточненных границах. Линия отражает значение лучшей найденной оценки, а столбцы показывают величину улучшения в моменты, когда алгоритм находил новое лучшее решение. Этот график показывает, что улучшения происходили дискретно: после отдельных удачных конфигураций следовали участки плато, характерные для стохастической оптимизации вычислительно затратной функции.
...
Для анализа результатов валидации наиболее показательны распределения, поскольку они агрегируют сравнение эталонных и оптимизированных параметров по числам контрольной выборки. Валидационные трассы отражают вклад отдельных чисел и повторов в эти распределения, однако именно графики распределений дают более компактное представление итогового эффекта. На рисунке 4.12 показано распределение валидационной метрики для GA. Оно демонстрирует смещение оптимизированной конфигурации относительно эталона в сторону меньших значений, что соответствует улучшению при минимизации.
...
На рисунке 4.13 приведено распределение времени валидации для GA. Этот график показывает, что улучшение итоговой метрики связано не только с изменением числа кривых, но и с непосредственным снижением времени выполнения факторизации на контрольной выборке. Смещение распределения оптимизированной конфигурации в сторону меньших значений подтверждает вывод о практическом ускорении метода.
...
На рисунке 4.14 приведено распределение числа кривых для PSO. Этот график выбран потому, что именно PSO дал максимальное сокращение среднего числа кривых. Распределение показывает практический смысл результата: при сохранении высокой доли успешных факторизаций оптимизированная конфигурация требует существенно меньше кривых, чем справочный режим GMP-ECM.
