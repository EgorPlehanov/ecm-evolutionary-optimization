...
На практике используется композитная оценка (composite score), учитывающий среднюю долю успешных запусков, среднее число кривых до исхода и среднее время до исхода. В реализованной системе оценка (score) минимизируется:
...
### 2.3.1. Случайный поиск (RS)

Случайный поиск (Random Search, RS) используется как базовый стохастический метод с минимальными предположениями о форме целевой функции [19]. На каждой итерации независимо сэмплируется кандидат в допустимой области, после чего выполняется внешняя оценка через GMP-ECM и обновляется best-so-far.

Алгоритмически RS в данной задаче важен по двум причинам: (1) он задает честный baseline для дорогой black-box оптимизации; (2) при малой размерности (две переменные) и сильном шуме измерений может быть конкурентоспособным с более сложными стратегиями.
...
На рисунке 2.1 показан простой цикл RS: генерация кандидата, проверка ограничений, дорогая внешняя оценка, условное обновление лучшего решения и переход к следующей итерации до исчерпания бюджета.

### 2.3.2. Дифференциальная эволюция (DE)
...
### 2.3.3. Генетический алгоритм (GA)
...
### 2.3.5. Байесовская оптимизация (BO)
