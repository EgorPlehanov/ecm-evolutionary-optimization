...
Дифференциальная эволюция (Differential Evolution, DE) является популяционным эволюционным методом для непрерывных black-box задач [16, 17]. Он использует мутацию через разности между особями, кроссовер и жадный отбор. Для рассматриваемой двумерной задачи DE привлекателен тем, что работает непосредственно в непрерывном лог-пространстве B1/B2 и обычно устойчив к негладкости функции.

Роевой алгоритм (Particle Swarm Optimization, PSO) моделирует движение частиц, каждая из которых учитывает собственный лучший результат и лучший результат роя [18]. Для двумерной области параметров PSO прост в реализации и может быстро находить перспективные зоны, но склонен сходиться к границам области, если функция поощряет экстремальные значения.

Случайный поиск (Random Search, RS) используется как простая эталонная стратегия. Его преимущество состоит в минимуме предположений о форме функции и простоте параллелизации. Для задач настройки гиперпараметров случайный поиск часто оказывается сильнее равномерной сетки, если эффективная размерность мала или важны не все параметры [19].

Байесовская оптимизация (Bayesian Optimization, BO) строит суррогатную модель дорогой функции и выбирает новые точки через acquisition-критерий [20, 21]. Этот подход особенно полезен при ограниченном бюджете оценок. В рассматриваемой задаче его эффективность может снижаться из-за шума, дискретизации параметров и нестационарности измерений, однако он важен как представитель model-based подходов.

Генетический алгоритм (Genetic Algorithm, GA) представляет эволюционный подход с популяцией, элитизмом, скрещиванием и мутациями. В данной задаче он применяется не к битовым строкам, а к вещественному представлению кандидатов в лог-пространстве. Его сильная сторона — способность поддерживать разнообразие популяции, а риск — преждевременная сходимость.
