...
Ключевые слова: факторизация целых чисел, ECM, GMP-ECM, параметры B1 и B2, эвристическая оптимизация, оптимизация «чёрного ящика», дифференциальная эволюция, генетический алгоритм, роевой алгоритм, байесовская оптимизация.
...
1. валидационной метрики — значение composite score на control-наборе;
...
Подбор параметров ECM естественно формализуется как дорогая стохастическая оптимизация «чёрного ящика» с ограничениями. Выбор эвристических методов обоснован отсутствием аналитического градиента, шумом измерений и высокой стоимостью оценки. Независимая валидация является обязательной частью постановки, поскольку оптимизация на train-наборе может привести к переобучению параметров под конкретные числа и seed.
...
В ходе работы выполнены поставленные задачи. Проведен аналитический обзор алгоритмов факторизации и практических реализаций ECM. Рассмотрены теоретические основы метода, роль стадий 1 и 2, а также справочные параметры GMP-ECM. Задача выбора B1/B2 формализована как стохастическая оптимизация «чёрного ящика» с ограничениями. Обосновано применение эвристических методов: случайный поиск, дифференциальная эволюция, генетический алгоритм, роевой алгоритм и байесовская оптимизация.
