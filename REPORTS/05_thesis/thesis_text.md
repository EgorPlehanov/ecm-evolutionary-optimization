...
Ключевые слова: факторизация целых чисел, ECM, GMP-ECM, параметры B1 и B2, эвристическая оптимизация, black-box оптимизация, дифференциальная эволюция, генетический алгоритм, роевой алгоритм, байесовская оптимизация.
...
Данная работа посвящена автоматизации выбора параметров B1 и B2 метода факторизации на эллиптических кривых. Объект исследования — алгоритм ECM и его реализация в GMP-ECM; цель — разработать систему подбора параметров ECM и оценить выигрыш относительно справочных значений GMP-ECM.

В ходе исследования выполнены анализ алгоритмов факторизации, формализация выбора B1/B2 как стохастической black-box оптимизации, разработка Python-пайплайна и вычислительные эксперименты. Использовались random search, differential evolution, genetic algorithm, particle swarm optimization и Bayesian optimization.

В результате разработана система генерации датасетов, запуска GMP-ECM, оптимизации, валидации и анализа. Для чисел с 20-значным простым делителем получено снижение validation score на 20,67-24,38%, времени — на 15,03-19,39%, числа кривых — на 50,44-63,33%. Результаты применимы в вычислительной теории чисел, криптоанализе и предварительной факторизации. Использованы Python 3, NumPy, SciPy, matplotlib, GMP-ECM, Git и ресурсы СКЦ СПбПУ.
...
The work is devoted to automating the selection of B1 and B2 parameters for the elliptic curve factorization method. The object is ECM and its implementation in GMP-ECM; the goal is to develop a system for ECM parameter tuning and evaluate its advantage over GMP-ECM reference parameters.
...
Цель работы — разработать и исследовать программную систему, которая автоматически подбирает параметры ECM и снижает эмпирическую оценку ожидаемой стоимости нахождения делителя по сравнению со справочными параметрами GMP-ECM.
...
Результаты вычислительных экспериментов были получены с использованием вычислительных ресурсов суперкомпьютерного центра Санкт-Петербургского политехнического университета Петра Великого (www.spbstu.ru).

# 2. Аналитический обзор методов факторизации и выбора параметров ECM

## 2.1. Задача факторизации и криптографический контекст

В простейшем случае задача факторизации формулируется как нахождение простых p и q по их произведению N = p q. В более общем случае требуется разложить составное число N на простые множители произвольной кратности. Для RSA выбираются большие простые p и q, а открытый модуль N публикуется. Если атакующий сможет эффективно найти p и q, он сможет восстановить закрытые параметры схемы. Поэтому практические рекорды факторизации и оценки стоимости GNFS имеют прямое значение для выбора размеров ключей [3, 4].
...
Классы алгоритмов факторизации приведены в таблице 2.1.
...
| Методы общего назначения | QS, GNFS | От размера N | Полная факторизация больших общих чисел |
| Вспомогательные методы | ECM в кофакторизации | От размера остаточных делителей | Постобработка и предварительная обработка |

## 2.2. Краткий обзор алгоритмов факторизации
...
Место ECM среди методов факторизации показано на рисунке 2.1.

```mermaid
flowchart LR
    A["Составное число N"] --> B{"Ожидается малый или средний делитель?"}
    B -- "Да" --> C["Специализированные методы"]
    C --> D["Pollard rho"]
    C --> E["Pollard p-1"]
    C --> F["ECM"]
    B -- "Нет / требуется полная факторизация" --> G["Методы общего назначения"]
    G --> H["QS"]
    G --> I["GNFS"]
    F --> J["Кофакторизация и предварительная факторизация"]
    I --> J
```

Рис. 2.1 Место ECM среди алгоритмов факторизации

## 2.3. Теоретические основы ECM
...
Общая схема ECM показана на рисунке 2.2.
...
Рис. 2.2 Блок-схема ECM со стадиями 1 и 2

## 2.4. Параметры B1 и B2
...
## 2.5. Практические реализации ECM
...
В данной работе GMP-ECM используется как внешний исполнитель. Это позволяет сосредоточиться не на переписывании арифметики эллиптических кривых, а на задаче автоматического выбора параметров и воспроизводимого анализа результатов. В экспериментах использовалась версия GMP-ECM 7.0.6, tag git-7.0.6, выпущенная 4 июля 2024 года [13].
...
ECM является хорошо изученным и практически значимым методом факторизации, особенно при поиске малых и средних делителей больших чисел. Его эффективность определяется вероятностной природой выбора кривых, гладкостью порядка группы и параметрами B1/B2. Существующие таблицы GMP-ECM являются надежным baseline, но не исключают возможности локальной настройки под конкретный класс задач. Это обосновывает переход от ручного выбора параметров к автоматизированной оптимизации.

# 3. Математическая постановка задачи и методы оптимизации

## 3.1. Формализация задачи

Пусть задан набор составных чисел D = {N_1, ..., N_L}. Для каждого N_j известен простой делитель целевого размера d, что необходимо для формирования датасета и последующей проверки результата. В реальном запуске оптимизатору известны только сами числа N_j и результаты работы GMP-ECM.

Управляемыми параметрами являются B1 и B2. Так как значения параметров меняются на порядки, оптимизация выполняется в логарифмическом пространстве:
...
На практике используется composite score, учитывающий среднюю долю успешных запусков, среднее число кривых до исхода и среднее время до исхода. В реализованной системе score минимизируется:

```text
score =
    max(0, 0.90 - success_rate) * 200000
    + mean_time_sec * 10000
    + mean_curves * 50.
```
...
## 3.2. Особенности целевой функции
...
Для защиты от переобучения используется разделение на train и control. Оптимизатор подбирает параметры на train-наборе, а итоговое сравнение с baseline выполняется на независимой control-выборке. Такой подход соответствует общей логике автоматической конфигурации алгоритмов, где качество найденной конфигурации должно подтверждаться вне обучающего набора.

## 3.3. Выбор методов оптимизации

В работе сравниваются пять методов.

Differential Evolution является популяционным эволюционным методом для непрерывных black-box задач [16, 17]. Он использует мутацию через разности между особями, кроссовер и жадный отбор. Для рассматриваемой двумерной задачи DE привлекателен тем, что работает непосредственно в непрерывном лог-пространстве B1/B2 и обычно устойчив к негладкости функции.

Particle Swarm Optimization моделирует движение частиц, каждая из которых учитывает собственный лучший результат и лучший результат роя [18]. Для двумерной области параметров PSO прост в реализации и может быстро находить перспективные зоны, но склонен сходиться к границам области, если функция поощряет экстремальные значения.

Random Search используется как простой baseline. Его преимущество состоит в минимуме предположений о форме функции и простоте параллелизации. Для задач настройки гиперпараметров случайный поиск часто оказывается сильнее равномерной сетки, если эффективная размерность мала или важны не все параметры [19].

Bayesian Optimization строит суррогатную модель дорогой функции и выбирает новые точки через acquisition-критерий [20, 21]. Этот подход особенно полезен при ограниченном бюджете оценок. В рассматриваемой задаче его эффективность может снижаться из-за шума, дискретизации параметров и нестационарности измерений, однако он важен как представитель model-based подходов.

Genetic Algorithm представляет эволюционный подход с популяцией, элитизмом, скрещиванием и мутациями. В данной задаче он применяется не к битовым строкам, а к вещественному представлению кандидатов в лог-пространстве. Его сильная сторона — способность поддерживать разнообразие популяции, а риск — преждевременная сходимость.

В целом выбранная постановка близка к автоматической конфигурации алгоритмов: подбирается не решение исходной математической задачи, а параметры вычислительного метода, качество которых затем проверяется на независимых экземплярах [22, 23].

Сравнение методов приведено в таблице 3.1.
...
|---|---|---|---|
| RS | Случайный поиск | Простота, дешевизна, устойчивый baseline | Нет направленного поиска |
...
## 3.4. Валидация и статистический анализ
...
1. validation score — значение composite score на control-наборе;
2. validation gain — относительное улучшение score относительно baseline;
...
Подбор параметров ECM естественно формализуется как дорогая стохастическая black-box оптимизация с ограничениями. Выбор эвристических методов обоснован отсутствием аналитического градиента, шумом измерений и высокой стоимостью оценки. Независимая валидация является обязательной частью постановки, поскольку оптимизация на train-наборе может привести к переобучению параметров под конкретные числа и seed.
...
## 4.2. Архитектура пакета

Программный комплекс реализован на Python и организован как пакет `ecm_optimizer` [24]. Основные модули:

- `ecm_optimizer/core/problem.py` — генерация простых чисел, составных N = p q и датасетов;
- `ecm_optimizer/core/ecm_runner.py` — запуск GMP-ECM и парсинг результата;
- `ecm_optimizer/core/fitness.py` — вычисление composite fitness и статистик;
- `ecm_optimizer/core/baseline.py` — выбор baseline-параметров GMP-ECM;
...
Архитектура системы показана на рисунке 4.1.
...
Рис. 4.1 Архитектура программной системы автоматизированного подбора параметров ECM

## 4.3. Генерация датасетов

Датасеты строятся из чисел вида N = p q, где p — простой делитель целевого размера, а q — кофактор заданной длины. Для каждого эксперимента формируются независимые train и control наборы. В manifest сохраняются исходные p, q и N, что позволяет проверять корректность найденных делителей и воспроизводить эксперимент.
...
## 4.6. Автоматизация экспериментов и HPC
...
Поток данных вычислительного эксперимента показан на рисунке 4.2.
...
Рис. 4.2 Поток данных вычислительного эксперимента
...
## 4.7. Система анализа результатов
...
## 5.1. Дизайн экспериментов

Основной обработанный класс экспериментов в текущей версии проекта — составные числа с 20-значным простым делителем и 30-значным кофактором. Для каждого запуска формировались train и control наборы. Baseline определялся по таблице GMP-ECM: для 20-значного делителя использовались B1 = 11000, B2 = 1900000 и ожидаемое число кривых около 74 [12].
...
## 5.2. Ретроспектива ранних запусков
...
## 5.3. Основные результаты для 20-значных делителей

В актуальном обработанном запуске `20_dset_20260518T233806Z_job7101768` использовались seed = 42, train-count = 80, control-count = 80, max-curves-per-n = 260 на этапе оптимизации и 600 на этапе валидации. Область поиска задавалась как B1 от 100 до 1000000, B2 от 10000 до 100000000, отношение B2/B1 не более 1000.

Сравнение baseline и оптимизированных параметров приведено в таблице 5.1.

Таблица 5.1
Сравнение baseline GMP-ECM и найденных параметров для 20-значных делителей

| Метод | Найденные параметры | Изменение score | Изменение времени | Изменение числа кривых | Success rate |
|---|---|---:|---:|---:|---:|
| BO | B1 = 41720, B2 = 2639911 | -24.38% | -19.39% | -57.55% | 1.0000 |
| DE | B1 = 32163, B2 = 2689055 | -23.26% | -19.15% | -50.44% | 1.0000 |
| RS | B1 = 35198, B2 = 3921020 | -20.67% | -15.03% | -57.91% | 1.0000 |
| PSO | B1 = 46568, B2 = 4510472 | -21.72% | -15.39% | -63.33% | 1.0000 |
| GA | B1 = 37035, B2 = 3399282 | -22.61% | -17.54% | -55.98% | 1.0000 |

Все методы улучшили baseline одновременно по трем ключевым метрикам: score, среднему времени и числу кривых. Наибольшее относительное снижение score показал BO (-24.38%), максимальное сокращение числа кривых показал PSO (-63.33%), а DE дал лучший баланс между абсолютным final objective и временем валидации. GA оказался близок к DE по качеству и также дал устойчивое улучшение.

Сводный анализ `analyze_20260519T100317Z_job7101779` показал медианный validation gain 22.611% и decision label `adopt` для размера делителя 20. Это означает, что для исследованного класса чисел автоматический подбор параметров можно считать практически успешным, хотя лидерство конкретного метода требует дополнительных повторов.
...
## 5.4. Сравнение методов по качеству и стоимости
...
Второе ограничение связано с типом чисел. Использовались полупростые числа N = p q с контролируемым размером p и q. Для специальных классов, например чисел Мерсенна, Ферма, чисел вида s^n +/- 1 и кофакторов из реальных NFS-задач, требуется отдельная проверка. Третье ограничение — стохастический характер ECM и оптимизаторов. Результаты следует подтверждать несколькими seed и независимыми control-наборами.
...
1. Использовать таблицы GMP-ECM как надежный стартовый baseline.
...
Эксперименты подтверждают практическую состоятельность автоматического подбора параметров ECM. Для 20-значных делителей все рассмотренные методы улучшили baseline GMP-ECM по validation score, времени и числу кривых. Медианный validation gain актуального сводного анализа составил 22.611%, а размер 20 получил решение `adopt`. Для окончательного расширения выводов требуется провести аналогичные эксперименты для 25-, 30- и более крупных делителей.
...
В ходе работы выполнены поставленные задачи. Проведен аналитический обзор алгоритмов факторизации и практических реализаций ECM. Рассмотрены теоретические основы метода, роль стадий 1 и 2, а также справочные параметры GMP-ECM. Задача выбора B1/B2 формализована как стохастическая black-box оптимизация с ограничениями. Обосновано применение эвристических методов: random search, differential evolution, genetic algorithm, particle swarm optimization и Bayesian optimization.

Разработана программная система на Python, обеспечивающая генерацию train/control датасетов, запуск GMP-ECM, расчет composite fitness, запуск оптимизаторов, независимую валидацию, автоматизированные эксперименты по JSON-планам и анализ результатов. Система сохраняет артефакты в JSON, CSV, PNG и Markdown-форматах, что обеспечивает воспроизводимость расчетов.

Экспериментальная проверка на составных числах с 20-значным простым делителем показала, что автоматический подбор параметров способен улучшить справочные параметры GMP-ECM. В актуальном запуске все методы снизили validation score на 20.67-24.38%, среднее время — на 15.03-19.39%, а среднее число кривых — на 50.44-63.33%. Сводный анализ показал медианный validation gain 22.611% и решение `adopt` для размера делителя 20.
