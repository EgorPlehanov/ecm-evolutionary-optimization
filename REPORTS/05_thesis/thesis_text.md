...
Программный комплекс реализован на Python и организован как пакет `ecm_optimizer` [24], в котором архитектура разделена на слои предметной логики, алгоритмов оптимизации, интерфейсов запуска и аналитической постобработки. Такое разбиение выбрано для того, чтобы изолировать вычислительное ядро (оценку параметров ECM) от сценариев экспериментов и от форматов итоговой отчетности.
...
|---|---|
| `seed` | Фиксирует инициализацию генераторов случайных чисел и обеспечивает воспроизводимость траектории поиска при повторном запуске. |
| `B1_min`, `B1_max` | Задают допустимый диапазон первой границы ECM и тем самым ограничивают область поиска по оси B1. |
| `B2_min`, `B2_max` | Задают допустимый диапазон второй границы ECM и ограничивают область поиска по оси B2. |
| `R_max` | Ограничивает отношение B2/B1, исключая заведомо нерациональные конфигурации с чрезмерно большим B2 при малом B1. |
| `max_evals` | Определяет верхнюю границу числа вычислений fitness и фактически задает вычислительный бюджет метода. |
| `time_budget_sec` | Вводит ограничение по времени оптимизации и используется как дополнительный критерий остановки длительных запусков. |
| `max_curves_per_n` | Ограничивает число кривых на одно N при оценке кандидата; влияет на точность оценки и ее стоимость. |
| `repeats` | Задает число повторов оценки на каждом N для снижения влияния стохастического шума ECM. |
| `timeout_sec` | Ограничивает длительность одного внешнего запуска GMP-ECM и переводит превышение лимита в нормализованный failure-статус. |
| `objective_weights` | Задает веса компонентов составной метрики (например, время, число кривых, штрафы), определяя приоритеты оптимизации. |
