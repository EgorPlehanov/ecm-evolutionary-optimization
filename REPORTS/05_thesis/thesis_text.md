...
Общая блок-схема ECM показана на рисунке 1.2.
...
В работе используется вариант DE/rand/1. Для каждого вектора $x_i^{(g)}$ выбираются три другие особи популяции: $x_{r1}^{(g)},x_{r2}^{(g)},x_{r3}^{(g)}$. Они должны быть различными. Обновляемый вектор в этот набор не входит. Затем строится мутантный вектор:
$v_i^{(g)}=x_{r1}^{(g)} + F\cdot\big(x_{r2}^{(g)}-x_{r3}^{(g)}\big)$.
Параметр $F\in(0,2)$ задаёт силу смещения. Чем больше $F$, тем дальше кандидат может уйти от базовой точки.

Дальше мутантный вектор смешивается с исходным решением. Так получается пробный вектор $u_i^{(g)}$. Каждая его координата берётся из $v_i^{(g)}$ с вероятностью $CR$. В остальных случаях сохраняется координата из $x_i^{(g)}$. Поэтому $CR\in[0,1]$ управляет степенью изменения родительского решения.

Перед оценкой кандидат проверяется на ограничения. Для задачи ECM важны условия $B_2\ge B_1$ и $\frac{B_2}{B_1}\le R_{\max}$. Если пробный вектор выходит за допустимую область, он корректируется или генерируется заново. Затем для него вычисляется $\mathrm{score}(u_i^{(g)})$ через запуск GMP-ECM.

Отбор выполняется жадно. Если пробный вектор не хуже родителя, он переходит в новое поколение: $x_i^{(g+1)}=u_i^{(g)}$. Если улучшения нет, сохраняется прежнее решение: $x_i^{(g+1)}=x_i^{(g)}$. Благодаря этому лучшее найденное значение score не ухудшается при смене поколений.
...
Генетический алгоритм (Genetic Algorithm, GA) использует другую логику поиска. Он не строит шаг через разность векторов, как DE. Новые точки появляются из уже оценённых родительских решений. Поэтому GA хорошо подходит для ситуации, где полезные свойства разных кандидатов можно объединять.

Индивид кодируется так же, как и в других методах: $x_i=(\log_{10}B_1,\log_{10}B_2)$. Оценка приспособленности получается через запуск GMP-ECM на обучающей выборке. Вещественное кодирование здесь естественно. Оба параметра подбираются в непрерывном логарифмическом пространстве.

Сначала формируется случайная популяция $P^{(0)}$. Затем все особи получают значения score. Для выбора родителей используется турнирная селекция. Из популяции случайно берётся небольшая группа решений. Родителем становится кандидат с наименьшим score в этой группе. Такой выбор усиливает хорошие решения, но не исключает остальные полностью. Это помогает сохранить разнообразие.

Для родителей $p_a$ и $p_b$ применяется арифметическое скрещивание:
$c=\alpha p_a+(1-\alpha)p_b$,
где $\alpha\sim U(0,1)$. Потомок лежит между двумя родителями в логарифмическом пространстве. После этого с вероятностью $p_m$ выполняется мутация:
$c'=c+\varepsilon$.
Компоненты $\varepsilon$ берутся из распределения $\mathcal{N}(0,\sigma^2)$. Такая мутация даёт локальное отклонение от уже найденных решений.

После скрещивания и мутации кандидат проверяется на ограничения. Координаты остаются в заданных диапазонах. При декодировании также проверяются условия $B_2\ge B_1$ и $\frac{B_2}{B_1}\le R_{\max}$. Затем потомок оценивается через GMP-ECM. Лучшие особи текущего поколения переносятся без изменений. Этот элитизм защищает удачные конфигурации от потери при случайных операциях.
...
где $\omega$ – коэффициент инерции, $c_1$ и $c_2$ – веса когнитивной и социальной компонент, $r_1,r_2\sim U(0,1)$ – независимые случайные множители. Формула сочетает три источника движения. Первый сохраняет часть прежней скорости. Второй направляет частицу к её собственному лучшему решению $pbest_i$. Третий учитывает лучший результат всего роя $gbest$.

После обновления позиции кандидат возвращается в допустимую область. Логарифмические координаты ограничиваются диапазонами поиска. При декодировании проверяются условия $B_2\ge B_1$ и $\frac{B_2}{B_1}\le R_{\max}$. Если шаг получается слишком большим, применяется velocity clamping. Это ограничивает скорость и делает поиск устойчивее.

Новая позиция оценивается через GMP-ECM. Если результат улучшает личный рекорд частицы, обновляется $pbest_i$. Если найдено лучшее значение среди всех частиц, обновляется $gbest$. После этого рой делает следующий шаг. Для подбора $B_1,B_2$ такая схема сочетает коллективный поиск и локальное уточнение.
...
Байесовская оптимизация (Bayesian Optimization, BO) рассчитана на дорогие целевые функции [21, 22]. В задаче настройки ECM это важно. Каждый кандидат требует запуска GMP-ECM. Поэтому метод должен получать максимум информации из небольшого числа оценок.

Кандидат задаётся тем же вектором $x=(\log_{10}B_1,\log_{10}B_2)$. Но BO дополнительно хранит модель $\hat f(x)$. Она приближает неизвестную зависимость score от параметров. Для каждой точки модель даёт средний прогноз $\mu(x)$ и неопределённость $\sigma(x)$. Рядом с уже проверенными точками неопределённость обычно ниже. В малоизученных областях она выше. Шум также повышает неопределённость. В этой задаче шум связан со случайными кривыми ECM, таймаутами и колебаниями времени выполнения.

В качестве суррогата обычно используют гауссовский процесс (Gaussian Process, GP). Его ядро $k(x,x')$ описывает связь между близкими точками. Набор наблюдений имеет вид $\mathcal{D}_t=\{(x_i,y_i)\}_{i=1}^{t}$, где $y_i=\mathrm{score}(x_i)$. По этим данным GP строит апостериорное распределение прогноза.

Следующий кандидат выбирается через acquisition-функцию $a(x\mid\mathcal{D}_t)$. Она должна совместить две цели. Первая цель – проверять точки с малым ожидаемым score $\mu(x)$. Вторая – исследовать области с большой неопределённостью $\sigma(x)$. Такая логика лежит в основе Expected Improvement (EI) и Upper/Lower Confidence Bound (UCB/LCB). После максимизации acquisition-функции получается точка $x_{t+1}$.

Найденная точка декодируется в $B_1,B_2$. Затем проверяются ограничения $B_2\ge B_1$ и $\frac{B_2}{B_1}\le R_{\max}$. После запуска GMP-ECM новая пара $(x_{t+1},y_{t+1})$ добавляется к наблюдениям. Модель обновляется, и цикл повторяется. За счёт этого BO не просто перебирает параметры, а постепенно уточняет картину целевой функции.
...
`Fitness evaluator` записывает результаты в JSON-артефакты. Затем эти данные поступают в блок анализа. Туда же передаются результаты валидации. На схеме этот этап отделён от оптимизации намеренно. Так разделяются настройка на train-наборе и проверка на control-наборе. На выходе формируются отчёты и графики в форматах Markdown, CSV и PNG. В главе 4 они используются для сравнения методов.

## 3.3. Генерация датасетов

Генератор строит наборы полупростых чисел вида $N = p q$. Эти наборы нужны для оптимизации и для независимой проверки. В конфигурации задаются размер простого делителя $p$, размер кофактора $q$, объёмы train- и control-наборов, а также `seed`. Значение `seed` фиксирует генератор псевдослучайных чисел. Поэтому повторный запуск с той же конфигурацией даёт тот же набор чисел.

Один элемент набора строится в несколько шагов. Сначала выбираются кандидаты $p$ и $q$ нужной длины. Затем проверяется их простота. После этого вычисляется $N = p q$ и проверяется его битовая длина. Если хотя бы одна проверка не пройдена, кандидат отбрасывается. Процесс продолжается, пока не будет получено нужное число элементов. Затем общий пул делится на train и control без пересечений. Это исключает утечку данных между настройкой и проверкой.
...
Таблица 4.1
Справочные параметры GMP-ECM и параметры, найденные для 20-значных делителей
...
При уточнённых границах все методы снова улучшили справочную конфигурацию. Валидационная метрика снизилась на 13,70-25,14%. Время уменьшилось на 14,30-21,04%. Число кривых снизилось на 9,71-62,42%. Лучший результат по валидационной метрике и времени дал GA. Наибольшее сокращение числа кривых получил PSO. DE показал ровные значения по всем трём показателям. BO также улучшил эталон, но его преимущество стало слабее, чем при широких границах.

Сводные статистики показывают близкий общий уровень улучшения на двух этапах. Для широких границ медианный выигрыш валидационной метрики составил 22,912%. Медианное итоговое значение целевой функции равно 24716,45. Для уточнённых границ эти значения составили 22,355% и 25799,23. При этом второй этап лучше разделил методы между собой. Наиболее заметно выделились GA и PSO.

Рисунки 4.1 и 4.2 сравнивают относительный выигрыш валидационной метрики. Они показывают эффект найденных конфигураций на независимой контрольной выборке. Отдельно показаны широкие и уточнённые границы поиска.
...
Рисунки 4.3 и 4.4 показывают связь между выигрышем и временем оптимизации. Это важно для практики. Лучшее значение метрики не всегда получается самым дешёвым поиском.
...
На рисунках 4.5 и 4.6 приведена сходимость трёх лучших методов. По ним видно, как быстро метод выходит к сильной конфигурации. Также видно, сохраняется ли улучшение при дальнейшем расходовании бюджета.
...
Отдельные графики оптимизации помогают увидеть ход поиска. Они показывают не только итоговые метрики. По ним можно проследить, какие значения $B_1$ и $B_2$ проверялись. Также видно, где появлялись лучшие оценки и как менялось текущее лучшее значение. Для подробного разбора выбраны GA, PSO и DE. GA дал лучший результат по метрике и времени. PSO сильнее всего сократил число кривых. DE использован как пример устойчивого популяционного поиска.

На рисунке 4.7 показана траектория GA в пространстве $B_1, B_2$. Это последовательность конфигураций, которые проверял метод. Видно, что поиск не остался около справочных параметров GMP-ECM. Он сместился к значениям $B_1$ порядка десятков тысяч и $B_2$ порядка нескольких миллионов. Это согласуется с таблицей 4.1. Для данного класса чисел такая область оказалась более выгодной.
...
Рисунок 4.8 показывает ту же задачу в координатах $B_1$ и $B_2/B_1$. Точки соответствуют проверенным конфигурациям. Цвет показывает значение целевой функции. Участки с меньшими значениями оценки образуют зоны интереса. Их можно рассматривать как направления для дальнейшего уточнения поиска.
...
Рисунок 4.9 показывает, когда GA находил новое лучшее значение. Линия задаёт текущую лучшую оценку. Столбцы показывают размер каждого улучшения. Прогресс идёт неравномерно. После удачных конфигураций появляются участки плато. Это ожидаемо для стохастического поиска с дорогой оценкой.
...
Распределения удобны для анализа валидации. Они показывают эффект не по одному числу, а по всей контрольной выборке. Трассы валидации дают больше деталей. Но распределения лучше передают итоговую картину. На рисунке 4.12 показана валидационная метрика для GA. Распределение оптимизированной конфигурации смещено к меньшим значениям. Для минимизируемой метрики это означает улучшение.
...
На рисунке 4.13 приведено время валидации для GA. Здесь видно, что эффект связан не только с числом кривых. Время факторизации на контрольной выборке тоже уменьшается. Распределение оптимизированной конфигурации сдвинуто влево. Это подтверждает практическое ускорение.
...
На рисунке 4.14 показано число кривых для PSO. Этот метод выбран потому, что сильнее всего снизил среднее число кривых. Распределение показывает практический смысл результата. При высокой доле успешных факторизаций оптимизированный режим требует меньше кривых, чем справочный GMP-ECM.
...
Метод нельзя выбирать только по выигрышу валидационной метрики. Нужно учитывать и стоимость оптимизации. BO хорошо проявил себя при широких границах. После сужения области его преимущество стало меньше. GA оказался сильнее по совокупности показателей. При широком поиске он дал лучшее итоговое значение целевой функции. При уточнённых границах он обеспечил максимальное снижение валидационной метрики (-25,14%) и времени (-21,04%). PSO выделился по числу кривых. При уточнённых границах оно уменьшилось на 62,42%.

DE дал наиболее ровный профиль. Валидационная метрика снизилась на 22,91% и 22,35%. Время уменьшилось на 18,90% и 17,92%. Число кривых снизилось на 49,25% и 51,69%. RS устроен проще остальных методов. Тем не менее он тоже улучшил справочную конфигурацию по ключевым метрикам. Его можно использовать как быстрый ориентир. Также он задаёт нижнюю границу пользы от автоматического подбора.

Практический вывод получается следующим. Для чисел с 20-значным делителем лучше всего выглядят GA, PSO и DE. GA предпочтителен по качеству и времени. PSO полезен, когда важно уменьшить число кривых. DE подходит как сбалансированный вариант. BO удобен для первичного исследования широкой области. После сужения диапазона ему нужна более аккуратная настройка бюджета и суррогатной модели.
