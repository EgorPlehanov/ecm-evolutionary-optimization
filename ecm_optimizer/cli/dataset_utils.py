...
def resolve_opt_result_file(
    ...
    *,
    ...
    if opt_result_ref is None:
        latest = (
            _latest_opt_result_file(dataset_results_dir)
            if dataset_results_dir is not None
            else _latest_opt_result_file_global(results_dir)
        )
        if latest is None:
            raise click.UsageError(
                f"No optimization result files found in {dataset_results_dir or results_dir}. "
...
def _latest_opt_result_file_global(results_dir: Path) -> Path | None:
