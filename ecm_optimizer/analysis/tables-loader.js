...
function roundNumber(value) {
  if (isDateString(value)) return formatDateTime(value);
  if (typeof value === "string" && /^\d{2}\.\d{2}\.\d{4} \d{2}:\d{2}:\d{2}$/.test(value)) return value;
...
async function renderAllTables(container) {
  container.innerHTML = "<div>Загрузка таблиц...</div>";
  let tables = [];
  const rawTables = container.dataset.tables;
  if (rawTables) {
    try {
      tables = JSON.parse(rawTables);
    } catch (err) {
      container.innerHTML = `<div>Ошибка парсинга data-tables: ${err}</div>`;
      return;
    }
  }
  try {
    const tablesHtml = await Promise.all(tables.map((table) => createTable(table)));
...
document.querySelectorAll("#tables-container[data-tables]").forEach((container) => {
