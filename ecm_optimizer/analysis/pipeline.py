...
_GROUP_CANDIDATES = ("divisor_size", "dataset", "method", "seed")
...
def _build_node_artifacts(
    *,
        ...
        run_rows.append(
            {
                ...
                "run_file": str(run.run_file),
                "validation_file": str(run.validation_file) if run.validation_file else "",
            }
        )
    ...
    report_lines = [
        f"# Analysis scope: `{_node_heading(node)}`",
        "",
        f"- Runs in scope: **{node_stats.run_count}**",
        f"- Median final objective: **{node_stats.median_objective:.6f}**",
        f"- IQR objective: **{node_stats.iqr_objective:.6f}**",
        f"- Success share (`objective <= {threshold:.6f}`): **{node_stats.success_share:.2%}**",
        f"- Median runtime: **{node_stats.median_time_sec:.3f} sec**",
    ]

    if node_stats.median_validation_gain_pct is not None:
        report_lines.append(f"- Median validation relative improvement: **{node_stats.median_validation_gain_pct:.3f}%**")
    else:
        report_lines.append("- Median validation relative improvement: **N/A** (validation files are missing in this scope)")

    report_lines.extend(
        [
            "",
            "## Tables",
            f"- [runs.csv](tables/{runs_table.name})",
            f"- [summary.csv](tables/{summary_table.name})",
        ]
    )

    if comparisons_table:
        report_lines.append(f"- [compare_by_{next_dimension}.csv](tables/{comparisons_table.name})")

    if plots:
        report_lines.extend(["", "## Plots"])
        for name, path in plots.items():
            report_lines.append(f"- {name}: ![{name}]({path.relative_to(node_dir)})")

    if node.children:
        report_lines.extend(["", "## Nested scopes"])
