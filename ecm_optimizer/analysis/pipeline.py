    ...
    *,
    ...
    report_lines.extend(
        [
            ...
            "}",
            "",
            "function datasetLabelFromPath(value) {",
            "  const parts = String(value).split('/').filter(Boolean);",
            "  if (!parts.length) return value;",
            "  const tail = parts[parts.length - 1];",
            "  if (tail.includes('.') && parts.length >= 2) return parts[parts.length - 2];",
            "  return tail;",
            "}",
            "",
            ...
            "  if (isPathLike(raw)) {",
            "    const label = columnName === 'dataset' ? datasetLabelFromPath(raw) : (raw.split('/').pop() || raw);",
