...
def _build_node_artifacts(
    *,
    ...
    source_run_links: set[str] = set()
    source_validation_links: set[str] = set()
    for run in sorted(node.runs, key=lambda item: (item.dataset, item.method, str(item.seed))):
        source_run_links.add(_project_relative(run.run_file))
        if run.validation_file:
            source_validation_links.add(_project_relative(run.validation_file))
        run_rows.append(
            {
                "dataset": run.dataset,
    ...
    report_lines = [
        ...
        "",
        "## Краткая сводка",
        f"- запусков в скоупе: **{node_stats.run_count}**",
    ...
    if node_stats.median_validation_gain_pct is not None:
        ...
        report_lines.append("- медианный прирост по validation: **N/A** (в скоупе нет файлов валидации)")
    ...
    if plots:
        ...
        report_lines.append("- Нет доступных графиков для текущего скоупа.")
    ...
    if source_run_links or source_validation_links:
        report_lines.extend(["", "## Источники (пути от корня проекта)"])
        if source_run_links:
            report_lines.append("- optimize-результаты:")
            for link in sorted(source_run_links):
                report_lines.append(f"  - `{link}`")
        if source_validation_links:
            report_lines.append("- validate-результаты:")
            for link in sorted(source_validation_links):
                report_lines.append(f"  - `{link}`")

    if node.children:
        report_lines.extend(["", "## Вложенные скоупы"])
