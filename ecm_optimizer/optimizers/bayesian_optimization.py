...
class BayesianOptimizationOptimizer(Optimizer):
    ...
    def optimize(self, *, ecm_bin: str, numbers: Iterable[int], config: OptimizationConfig) -> OptimizationResult:
        ...
        for _ in range(initial_samples):
            ...
            evaluated.append(evaluate_candidate(x_log=x, ecm_bin=ecm_bin, numbers=numbers, config=config, progress=progress))
        ...
        for iteration in range(1, iterations + 1):
            ...
            evaluated.append(evaluate_candidate(x_log=candidate, ecm_bin=ecm_bin, numbers=numbers, config=config, progress=progress))
