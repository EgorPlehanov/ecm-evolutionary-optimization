...
def generate_analysis_artifacts(
    *,
    ...
    lines.extend(
        [
            "",
            "## Интерактивные таблицы (HTML + JS)",
            "<script>",
            "async function loadCsvTable(containerId, csvPath) {",
            ...
            "  html += '</tr></thead><tbody>';",
            "  rows.slice(1).forEach(r => { html += '<tr>'; r.forEach(c => html += `<td>${c}</td>`); html += '</tr>'; });",
            "  html += '</tbody></table>';",
    ...
    for key, path in sorted(files.items()):
        ...
        lines.extend(
            [
                f"<details><summary>Показать таблицу: {key}</summary>",
                f"<div id=\"{container_id}\">Загрузка...</div>",
                "<script>",
