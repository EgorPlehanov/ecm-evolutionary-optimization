...
def _attention_flags(
    ...
    *,
    ...
    if history and config:
        ...
        b1_boundary_share = _boundary_share(b1_values, float(b1_min) if b1_min is not None else None, float(b1_max) if b1_max is not None else None)
        b2_boundary_share = _boundary_share(b2_values, float(b2_min) if b2_min is not None else None, float(b2_max) if b2_max is not None else None)
        ratio_boundary_share = _boundary_share(ratio_values, 0.0, float(ratio_max) if ratio_max is not None else None)
        ...
        if optimized:
            ...
            if best_b1 is not None and b1_min is not None and b1_max is not None:
                ...
                flags["best_b1_on_boundary"] = {
                    ...
                    "value": float(best_b1),
                    "threshold": f"within 2% of [{b1_min}, {b1_max}]",
            ...
            if best_b2 is not None and b2_min is not None and b2_max is not None:
                ...
                flags["best_b2_on_boundary"] = {
                    ...
                    "value": float(best_b2),
                    "threshold": f"within 2% of [{b2_min}, {b2_max}]",
            ...
            if best_b1 not in (None, 0, 0.0) and best_b2 is not None and ratio_max is not None:
                ...
                flags["best_ratio_on_boundary"] = {
                    ...
                    "value": best_ratio,
                    "threshold": f"within 2% of ratio_max={ratio_max}",
