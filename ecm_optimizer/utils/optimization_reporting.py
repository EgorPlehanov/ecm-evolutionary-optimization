    ...
    *,
    ...
    for key, path in sorted(files.items()):
        ...
        if not str(rel).endswith(".csv"):
            continue
        container_id = f"table_{key}"
        table_specs.append((key, str(rel), container_id))
        lines.extend(
            [
                "<details>",
                f"<summary>Показать таблицу: <a href=\"{rel}\">{key}</a></summary>",
                "",
    ...
    if table_specs:
        ...
        for _, rel, container_id in table_specs:
            lines.append(f"loadCsvTable('{container_id}', '{rel}');")
        lines.append("</script>")
