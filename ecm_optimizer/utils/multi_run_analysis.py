...
def _plot_overlay_curves(plt: Any, grouped_runs: dict[str, list[RunRecord]], output_dir: Path, options: AnalysisOptions) -> dict[str, Path]:
    plots: dict[str, Path] = {}

    plt.figure(figsize=(11, 6))
    for label, runs in sorted(grouped_runs.items()):
        x, y = _median_curve_by_eval(runs, options.max_eval_points)
        if x:
            plt.plot(x, y, linewidth=1.8, label=label)
    plt.title("Overlay convergence (median best_so_far by eval)")
    plt.xlabel("Evaluation")
    plt.ylabel("Best objective so far")
    plt.grid(alpha=0.3)
    plt.legend(fontsize=8)
    path_eval = output_dir / "overlay_convergence_eval.png"
    plt.tight_layout()
    plt.savefig(path_eval, dpi=160)
    plt.close()
    plots["overlay_convergence_eval"] = path_eval

    plt.figure(figsize=(11, 6))
    for label, runs in sorted(grouped_runs.items()):
        x, y = _median_curve_by_time(runs, options.max_time_points)
        if x:
            plt.plot(x, y, linewidth=1.8, label=label)
    plt.title("Overlay convergence (median best_so_far by time)")
    plt.xlabel("Elapsed seconds")
    plt.ylabel("Best objective so far")
    plt.grid(alpha=0.3)
    plt.legend(fontsize=8)
    path_time = output_dir / "overlay_convergence_time.png"
    plt.tight_layout()
    plt.savefig(path_time, dpi=160)
    plt.close()
    plots["overlay_convergence_time"] = path_time
...
def _plot_success_profile(plt: Any, grouped_runs: dict[str, list[RunRecord]], output_dir: Path, threshold: float) -> tuple[Path, Path]:
    ...
    for label, runs in sorted(grouped_runs.items()):
        ...
        if eval_grid:
            plt.plot(eval_grid, shares, linewidth=1.8, label=label)
    plt.title(f"Success profile by eval (threshold <= {threshold:.6f})")
    plt.xlabel("Evaluation")
    plt.ylabel("Share of successful runs")
    plt.ylim(0, 1.05)
    plt.grid(alpha=0.3)
    plt.legend(fontsize=8)
    eval_out = output_dir / "success_profile_eval.png"
    plt.tight_layout()
    plt.savefig(eval_out, dpi=160)
    plt.close()
    ...
    for label, runs in sorted(grouped_runs.items()):
        ...
        for point in time_grid:
            ...
            shares.append(success / len(runs) if runs else 0.0)
        if time_grid:
            plt.plot(time_grid, shares, linewidth=1.8, label=label)
    plt.title(f"Success profile by time (threshold <= {threshold:.6f})")
    plt.xlabel("Elapsed seconds")
    plt.ylabel("Share of successful runs")
    plt.ylim(0, 1.05)
    plt.grid(alpha=0.3)
    plt.legend(fontsize=8)
    time_out = output_dir / "success_profile_time.png"
    plt.tight_layout()
    plt.savefig(time_out, dpi=160)
    plt.close()
    return eval_out, time_out
...
def generate_multi_run_artifacts(
    *,
    ...
    run_files = _discover_run_files(input_paths, experiments_root)
    ...
    summary = _build_summary(grouped_runs, threshold=threshold, group_by=options.group_by, input_paths=input_paths)

    plots: dict[str, Path] = {}
    plt = _load_matplotlib_pyplot()
    if plt is not None:
        plots.update(_plot_overlay_curves(plt, grouped_runs, output_dir, options))
        ...
        success_eval, success_time = _plot_success_profile(plt, grouped_runs, output_dir, threshold)
        plots["success_profile_eval"] = success_eval
        plots["success_profile_time"] = success_time
    ...
    analyzer_params_file = output_dir / "analyzer_run_params.json"
    analyzer_payload = {
    ...
    write_json_with_meta(analyzer_params_file, analyzer_payload, command="analyze")

    return MultiRunArtifacts(
        summary_file=summary_file,
        analyzer_params_file=analyzer_params_file,
        plots=plots,
