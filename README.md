...
- Каркас Python-пайплайна для:
  ...
  - оценки composite-фитнеса с нормализацией относительно baseline (`B1=11000`, `B2=220000`):
...
Итоговый `score` (минимизируется) считается в относительных дельтах к baseline:
- `Δtime = (time - time_baseline) / time_baseline`;
- `Δcurves = (curves - curves_baseline) / curves_baseline`;
- `Δsuccess = success_baseline - success`.

Далее применяется композиция:
- штраф за недобор success относительно baseline (с допуском 1 п.п.);
- штраф за деградацию времени (время не должно становиться хуже baseline);
- линейный вклад `Δtime` (основной) и `Δcurves` (вторичный).

Эта схема защищает от сценария «лучший success/curves ценой заметного ухудшения времени».
